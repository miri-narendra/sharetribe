...
function initialize_send_booking_request_form(locale) {
  ...
  $(form_id).validate({
    rules: {
      ...
      "listing_conversation[content]": {required: false, minlength: 1}
    },
    submitHandler: function(form) {
      disable_and_submit(form_id, form, "false", locale);
...
function initialize_new_offer_form(
  update_invoice_url,
  commission_percentage
  ) {
  ...
  $('#listing_id, #input-housekeeping-kit').on('change', { update_invoice_url: update_invoice_url, commission_percentage: commission_percentage }, offer_form_update_invoice);
  $('.cart_other_fees_wrapper').on('blur', '.cart_other_fee_amount_field', { update_invoice_url: update_invoice_url, commission_percentage: commission_percentage }, offer_form_update_invoice);
  $('#start-on, #end-on, #total-miles, #total_generator-hours, '+
    '#transaction_cart_attributes_security_deposit, '+
    '#transaction_cart_attributes_pickup_dropoff_fee, '+
    '#transaction_cart_attributes_discount'
  ).on('blur', { update_invoice_url: update_invoice_url, commission_percentage: commission_percentage }, offer_form_update_invoice);

  attach_payment_simple_form_listeners(commission_percentage);
  offer_form_update_invoice({data:{ update_invoice_url: update_invoice_url, commission_percentage: commission_percentage }});
}

function attach_payment_simple_form_listeners(commission_percentage){
  if (commission_percentage != null) {
    $(".trigger-focusout").bind('keyup', {commission_percentage: commission_percentage},
        function(event){ update_simple_form_price_fields(event.data.commission_percentage)
    });
    /*
    $(form_id).validate({
      rules: {
...
function offer_form_update_invoice(event){
  update_invoice_url = event.data.update_invoice_url
  var valuesToSubmit = $('#new_transaction').serialize();
  $.ajax({
      type: "POST",
      url: update_invoice_url, //sumbits it to the given url of the form
      data: valuesToSubmit,
      dataType: "script" // you want a difference between normal and ajax-calls, and json is standard
  }).success(function(data){
      eval(data);
      attach_payment_simple_form_listeners(event.data.commission_percentage);
...
function initialize_signup_form(locale, username_in_use_message, invalid_username_message, email_in_use_message, invalid_invitation_code_message, invalid_phone_number_message, name_required, invitation_required) {
  ...
  $(form_id).validate({
    ...
    rules: {
      ...
      "person[phone_number]": {required: true, maxlength: 30, remote: '/people/check_phone_number_validity'},
    ...
    messages: {
      ...
      "person[phone_number]": { remote: invalid_phone_number_message }
    },
...
function initialize_update_profile_info_form(locale, person_id, invalid_phone_number_message, name_required) {
...
function initialize_pending_consent_form(email_invalid_message, phone_number_invalid_message, invitation_required, invalid_invitation_code_message) {
  $('#help_invitation_code_link').click(function(link) {
    $('#help_invitation_code').lightbox_me({centered: true, zIndex: 1000000 });
  });
  $('#terms_link').click(function(link) {
  ...
  $('#pending_consent_form').validate({
    ...
    rules: {
      ...
      "form[phone_number]": {required: true, maxlength: 30, remote: '/community_memberships/check_phone_number_validity'},
    ...
    messages: {
      ...
      "form[phone_number]": { remote: phone_number_invalid_message },

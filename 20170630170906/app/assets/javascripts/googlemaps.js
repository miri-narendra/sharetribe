...
function showCityCountryLocationInfo(address)
{
    var _location_element = $("#city-address-location");
    if (_location_element.length > 0) {
        var gcoder = new google.maps.Geocoder();
        gcoder.geocode({"address": address}, function (place, info) {
            if (info == google.maps.GeocoderStatus.OK) {
                var
                    _city = getAddressComponentByType(place[0], 'locality'),
                    _state = getAddressComponentByType(place[0], 'administrative_area_level_1');
                var _location = [_city, _state].filter(function (i) {
                    return i !== undefined
                }).join(', ');
                if (_location) {
                    _location_element.html(_location);
                    _location_element.parent().removeClass('hide');
                }
            }
        });
    }
...
function getAddressComponentByType(location, type)
{
    for (var i = 0; i < location.address_components.length; i++) {
        console.log(location.address_components[i].types[0]);
        if (location.address_components[i].types[0] === type) {

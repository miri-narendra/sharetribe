...
class PersonMailer < ActionMailer::Base
  ...
  def new_payment_split_owner(payment, transaction, community)
    @email_type =  "email_about_new_payments"
    @payment = payment
    @transaction = transaction
    @community = community
    ...
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
                     :subject => t("emails.new_payment_split_owner.subject"))
    end
  end

  def new_payment_split_renter(payment, transaction, community)
    @email_type =  "email_about_new_payments"
    @payment = payment
    @transaction = transaction
    @community = community
    ...
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
                     :subject => t("emails.new_payment_split_renter.subject"))
    end
  end
  ...
  def transactions_stuck_renter_notification(community)
    @community = community
    pending_transactions = MarketplaceService::Transaction::Query
    ...
    pending_transactions.each do |transaction|
      ...
      set_up_layout_variables(@recipient, @community, @email_type)
      with_locale(@recipient.locale, community.locales.map(&:to_sym), @community.id) do
        bcc_emails = @community.admin_emails << "accounting@privatemotorhomerental.com"
        premailer_mail(:to => @recipient.confirmed_notification_emails_to,
             :from => community_specific_sender(@community),
             :bcc => bcc_emails,
             :subject => t("emails.payment_reminder.reminder_to_confirm_rental"))
      end
    end
  end

  def transactions_stuck_admin_notification(community)
    @community = community
    @pending_transactions = MarketplaceService::Transaction::Query
      .transactions_pending()

    stuck_transaction_count = @pending_transactions.count
    ...
    premailer_mail(:to => @community.admin_emails,
         :from => community_specific_sender(@community),
         :subject => "PMH [#{stuck_transaction_count}] stuck transactions")
  end

  def scheduled_payments_stuck_admin_notification(community)
    @community = community

    payments_query = Payment
      .joins('LEFT JOIN transactions ON payments.transaction_id = transactions.id')
      .joins('LEFT JOIN payment_splits ON payments.id = payment_splits.payment_id')
      .where(community_id: @community.id, status: [:partial, :pending])
      .where('transactions.current_state = ?', :accepted)
    stuck_scheduled_payments_splitted = payments_query
      .where('payment_splits.status = ? AND payment_splits.due_on IS NOT NULL AND payment_splits.due_on <= ?',
        :pending, 5.days.ago)
      .group(:transaction_id)
    stuck_scheduled_payments_not_splitted = payments_query
      .where('payment_splits.status IS NULL AND payment_splits.due_on IS NULL AND payments.created_at <= ?',
        7.days.ago) # no payments yet, so 2 days for first one + 5 days gap = the date to remind
    @stuck_scheduled_payments = (stuck_scheduled_payments_splitted + stuck_scheduled_payments_not_splitted)
    ...
    premailer_mail(:to => @community.admin_emails,
         :from => community_specific_sender(@community),
         :subject => "PMH stuck transactions")
  end
  ...
  def transaction_confirmed(conversation, community)
    ...
    recipient = detect_recipient_for_canceled(conversation, conversation.seller) 
    set_up_layout_variables(recipient, community, @email_type)
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
  ...
  def escrow_canceled_to(conversation, community, to, recipient)
  ...
  def escrow_canceled(conversation, community)
    recipient = detect_recipient_for_canceled(conversation, conversation.seller)
    escrow_canceled_to(conversation, community, recipient.confirmed_notification_emails_to, recipient)
  end

  def admin_escrow_canceled(conversation, community)
    recipient = detect_recipient_for_canceled(conversation, conversation.seller)
    escrow_canceled_to(conversation, community, community.admin_emails.join(","), recipient)
  end
  ...
  # Remind users to pay
  def payment_reminder(conversation, recipient, community, template = 'payment_reminder')
    @email_type = "email_about_payment_reminders"
    ...
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      ...
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
                     :bcc => bcc_emails,
                     :subject => t("emails.payment_reminder.reminder_to_confirm_rental")) do |format|
        format.html { render template }
      end
    end
  end

  # Remind owner to follow up with renter about the need to pay
  def follow_up_reminder(transaction, template)
    @email_type = "email_about_payment_reminders"
    @transaction = transaction
  ...
  def upcoming_booking_reminder(transaction, recipient, community)
    @email_type = "email_about_payment_reminders"
    set_up_layout_variables(recipient, community, @email_type)
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      @transaction = transaction
      ...
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
        :from => community_specific_sender(community),
        :subject => t("emails.payment_reminder.upcoming_booking_starts_owner_reminder.subject"))
    end
  end
  ...
  # Remind author and admin to refund security deposit
  def deposit_refund_reminder(conversation, recipient, community)
    @email_type = "email_about_payment_reminders"
    set_up_layout_variables(recipient, community, @email_type)
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      @conversation = conversation
      ...
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
                     :bcc => bcc_emails,
                     :subject => t("emails.deposit_refund_reminder.subject", listing_title: @conversation.listing.title )) do |format|
        format.html { render 'deposit_refund_reminder' }
      end
    end
  end
  ...
  def new_member_notification(person, community, email)
    ...
    @person = person
    @email = email
    person_type = (person.is_owner? ? "owner" : "renter")
    premailer_mail(:to => @community.admin_emails,
         :from => community_specific_sender(@community),
         :subject => "New #{person_type}")
  end

  def new_listing_notification(listing, community)
    @community = community
    @listing = listing
    ...
    premailer_mail(:to => @community.admin_emails,
         :from => community_specific_sender(@community),
         :subject => "PMH New listing")
  end


  def new_transaction_notification(owner, transaction, community, owner_email)
    @owner = owner
    @community = community
    @transaction = transaction
    ...
    premailer_mail(:to => @community.admin_emails,
         :from => community_specific_sender(@community),
         :subject => "PMH Rental Request")
  end

  def request_accepted_notification(transaction, community)
    @community = community
    @transaction = transaction
    premailer_mail(:to => @community.admin_emails,
         :from => community_specific_sender(@community),
         :subject => "PMH Rental Request Accepted")
  end

  def new_payment_notification(payment, transaction, community)
    @community = community
    @transaction = transaction
    @payment = payment
    @last_paid_split = payment.last_paid_split
    @url_params = {}
    @url_params[:host] = "#{@community.full_domain}"
    @url_params[:locale] = 'en'
    @url_params.freeze # to avoid accidental modifications later
    @transaction_url = person_transaction_url(transaction.starter, @url_params.merge({:id => transaction.id.to_s}))
    emails = @community.admin_emails << "accounting@privatemotorhomerental.com"
    premailer_mail(:to => emails,
         :from => community_specific_sender(@community),
         :subject => "PMH Payment made")
  end

  def stripe_not_linked_notification(community)
    @community = community
    @stripe_not_linked_owners = @community.owners.select { |owner| owner.stripe_account.nil? }
    @stripe_event_logs_weekly = StripeEventLog.where(event_type: 'PaymentNoStripeAccount')
                                              .where('created_at >= ?', 1.week.ago)
    emails = @community.admin_emails << "accounting@privatemotorhomerental.com"
    premailer_mail(:to => emails,
         :from => community_specific_sender(@community),
         :subject => "PMH Weekly update about unlinked Stripe accounts")
  end
  ...
  def welcome_email(person, community, regular_email=nil, test_email=false)
    ...
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      ...
      template = (@recipient.is_owner? ? 'person_mailer/welcome_owner_email' : 'person_mailer/welcome_email')
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
                     :subject => subject
                    ) do |format|
        format.html { render :layout => 'email_blank_layout', :template => template }
      end
    end
  end
  ...
  def detect_recipient_for_canceled(transaction, recipient)
    return recipient unless transaction.current_state == 'canceled'
    transition = transaction.transaction_transitions.where(to_state: 'canceled').first
    return recipient unless transition && transition.metadata && transition.metadata['sender']
    sender = Person.where(id: transition.metadata['sender']).first 
    sender ? @conversation.other_party(sender) : recipient
  end
end

...
class TransactionMailer < ActionMailer::Base
  ...
  def transaction_created(transaction)
    ...
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      premailer_mail(
        mail_params(recipient, community, t("emails.new_transaction.subject", :sender_name => sender_name))) do |format|
        format.html {
          render locals: {
                   recipient: recipient,
                   reply_url: reply_url,
  ...
  def offer_created(transaction)
    community = transaction.community

    recipient = transaction.starter
    sender = transaction.author
    sender_name = sender.name(community)
    ...
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      premailer_mail(
        mail_params(recipient, community, t("emails.new_offer.you_have_a_new_offer", :sender_name => sender_name))) do |format|
        format.html {
          render locals: {
                   recipient: recipient,
                   reply_url: reply_url,
  ...
  def inquiry_transaction_created(transaction)
    community = transaction.community
    ...
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      premailer_mail(
        mail_params(recipient, community, t("emails.new_inquiry.you_have_a_new_inquiry", :sender_name => sender_name))) do |format|
        format.html {
          render locals: {
                   recipient: recipient,
                   reply_url: reply_url,
                   sender: sender,
                   sender_name: sender_name,
                   sender_email: sender.emails.best_address,

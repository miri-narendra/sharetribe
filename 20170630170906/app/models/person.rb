...
class Person < ActiveRecord::Base
  ...
  validates_length_of :phone_number, :maximum => 25, :allow_nil => false, :allow_blank => false
  ...
  def is_owner?
    community_membership.is_owner?
  end

  def allow_inquiry?
    community_membership.allow_inquiry?
  end
  ...
  def commission_percent_for(community)
    community_memberships.where("community_memberships.community_id" => community.id).first.commission_percent
  end

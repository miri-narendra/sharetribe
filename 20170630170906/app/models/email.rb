...
class Email < ActiveRecord::Base
  ...
  scope :confirmed_and_notifications, -> { confirmed.where(send_notifications: true) }
  scope :confirmed, -> { where('confirmed_at IS NOT NULL') }
  scope :pending,   -> { where('confirmed_at IS NULL') }

  def self.best_address
    (confirmed_and_notifications.first || confirmed.first || pending.first).address
  end

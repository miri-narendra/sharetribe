...
class StripePayment < Payment
  attr_accessible :stripe_transaction_id, :currency, :sum

  monetize :sum_cents, allow_nil: true, with_model_currency: :currency
  monetize :adjusted_sum_cents, allow_nil: true, with_model_currency: :currency
  ...
  def default_sum(transaction_hash, vat=0)
    self.sum = (transaction_hash[:cart] ? transaction_hash[:cart][:total_sum] : nil ) ||
               transaction_hash[:checkout_total] ||
               transaction_hash[:item_total] ||
               transaction_hash[:listing_price]
  end
end

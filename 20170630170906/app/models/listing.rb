...
class Listing < ActiveRecord::Base
  ...
  validates :open, inclusion: { in: [false] }, if: :"not_approved?" #allow publising only if admin has approved listing
  ...
  def self.currently_open(status="open")
    ...
    when "all"
      where([])
    when "open"
      where(["is_approved ='1' AND open = '1' AND (valid_until IS NULL OR valid_until > ?)", DateTime.now])
    when "closed"
      where(["is_approved = '0' OR open = '0' OR (valid_until IS NOT NULL AND valid_until < ?)", DateTime.now])
    end
  end
  ...
  def closed?
    !is_approved? || !open? || (valid_until && valid_until < DateTime.now)
  end

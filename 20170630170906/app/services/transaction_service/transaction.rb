module TransactionService::Transaction
  ...
  def create(opts, paypal_async: false)
    ...
    payment_gateway_name = tx[:payment_gateway]
    payment_process_name = (payment_gateway_name == :none ? :none : tx[:payment_process])

    tx_process = tx_process(payment_process_name)
    gateway_adapter = gateway_adapter(payment_gateway_name)
  ...
  def schedule_payment_reminders(current_community, transaction, skip_first_reminder: false)
    #figure out in how many days whould we send payment reminders
    payment_schedule = TransactionService::PaymentSchedule.get(community_id: current_community.id, transaction_id: transaction.id)
    first_reminder = 2
    second_payment_is_due_on = payment_schedule.second_payment_is_due_on
    skip_second_reminder = second_payment_is_due_on.nil?
    before_second_payment = (second_payment_is_due_on - Date.current).to_i unless skip_second_reminder
    before_security_deposit = (payment_schedule.security_deposit_is_due_on - Date.current).to_i

    unless skip_first_reminder
      Delayed::Job.enqueue(PaymentReminderJob.new(transaction.id, transaction.payment.payer.id, current_community.id, 'payment_reminder'), :priority => 9, :run_at => adjust_time(first_reminder.days.from_now))
    end
    if !skip_second_reminder && before_second_payment > first_reminder
      Delayed::Job.enqueue(PaymentReminderJob.new(transaction.id, transaction.payment.payer.id, current_community.id, 'second_payment_reminder'), :priority => 9, :run_at => adjust_time(before_second_payment.days.from_now))
      # add job to send the message to owner if second payment was not paid 5 days after the second payment due date
      Delayed::Job.enqueue(FollowUpReminderJob.new(transaction.id, current_community.id, 'owner_second_payment_reminder'), :priority => 9, :run_at => adjust_time(before_second_payment.days.from_now + 5.days))
    end
    if before_security_deposit > first_reminder
      Delayed::Job.enqueue(PaymentReminderJob.new(transaction.id, transaction.payment.payer.id, current_community.id, 'security_deposit_payment_reminder'), :priority => 9, :run_at => adjust_time(before_security_deposit.days.from_now))
      # add job to send the message to owner if Security Deposit payment was not paid 5 days after SD due date
      Delayed::Job.enqueue(FollowUpReminderJob.new(transaction.id, current_community.id, 'owner_security_deposit_payment_reminder'), :priority => 9, :run_at => adjust_time(before_security_deposit.days.from_now + 5.days))
    end

    # send the notification email to listing owner 5 days before the booking starts
    reminder_days_before = 5
    reminder_at = payment_schedule.booking.start_on - reminder_days_before.day
    if reminder_at > DateTime.now
      Delayed::Job.enqueue(UpcomingBookingReminderJob.new(transaction.id, current_community.id), :priority => 9, :run_at => reminder_at)
    end
  end

  HOUR_TO_REMIND = 13 # cron for pending payments runs at 06:00am US Eastern Time = 10:00am UTC, schedule reminders after it
  def adjust_time(date_time)

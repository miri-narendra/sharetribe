...
class BraintreeSaleService
  def initialize(payment, payment_params)
    ...
    @amount = @payment_object.sum.cents.to_f / subunit_to_unit
    @service_fee = @payment_object.commission.cents.to_f / subunit_to_unit
  ...
  def save_transaction_id!(result)
    @payment_object.update_attributes(braintree_transaction_id: result.transaction.id)
  end

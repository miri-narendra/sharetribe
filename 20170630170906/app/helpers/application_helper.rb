...
module ApplicationHelper
  ...
  def transaction_statuses
    TransactionProcessStateMachine.states
  end

  def transaction_payment_statuses
    Transaction::VALID_PAYMENT_STATUSES
  end
  ...
  def admin_links_for(community)
    ...
    links += [
      ...
      {
        :topic => :manage,
        :text => t("admin.communities.manage_members.manage_owners"),
        :icon_class => icon_class("community"),
        :path => owners_admin_community_community_memberships_path(@current_community, sort: "join_date", direction: "desc"),
        :name => "manage_owners"
      },
      {
        :topic => :manage,
        :text => t("admin.communities.manage_members.manage_renters"),
        :icon_class => icon_class("community"),
        :path => renters_admin_community_community_memberships_path(@current_community, sort: "join_date", direction: "desc"),
        :name => "manage_renters"
      },
      {
        :topic => :manage,
        :text => t("admin.communities.manage_listings.manage_listings"),
      ...
      },
      {
        ...
        :path => admin_community_transactions_path(@current_community, sort: "started", direction: "desc"),
        :name => "transactions"
      },
      {
        :topic => :configure,
  ...
  def payment_settings_path(gateway_type, person)
    ...
    elsif gateway_type == :stripe
      show_stripe_settings_payment_path(person)
    end
  end
  ...
  def payment_settings_url(gateway_type, person, url_params)
    if gateway_type == :braintree
      ...
      show_stripe_settings_payment_url(person, url_params.merge(locale: person.locale))
    end
  end

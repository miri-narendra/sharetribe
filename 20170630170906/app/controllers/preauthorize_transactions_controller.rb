class PreauthorizeTransactionsController < ApplicationController
  ...
  def book
    ...
    booking_data = verified_booking_data(params[:start_on], params[:end_on], minimum_duration)

    vprms = view_params(listing_id: params[:listing_id],
                        quantity: booking_data[:duration],
                        shipping_enabled: delivery_method == :shipping)

    cprms = cart_params(cart: params[:cart])
    ...
    price_break_down_locals = TransactionViewUtils.price_break_down_locals({
      ...
      due_now_sum: schedule.due_now_sum,
  ...
  def booked
    ...
    transaction_response = create_preauth_transaction(
      ...
      listing: @listing,
      user: @current_user,
      listing_quantity: DateUtils.duration_nights(preauthorize_form.start_on, preauthorize_form.end_on, minimum_duration),
      content: preauthorize_form.content,
      use_async: request.xhr?,
      delivery_method: delivery_method,
  ...
  def verified_booking_data(start_on, end_on, minimum_duration)
    ...
    if !booking_form.valid?
      ...
      booking_form.to_hash.merge({
        duration: DateUtils.duration_nights(booking_form.start_on, booking_form.end_on, minimum_duration)
      })
    end
  end

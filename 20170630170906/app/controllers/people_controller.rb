class PeopleController < Devise::RegistrationsController
  ...
  def new
    ...
    @person = if params[:person] then
      Person.new(params[:person].slice(:given_name, :family_name, :email, :username, :is_owner))
    else
      Person.new()
    end
  ...
  def create
    ...
    if @current_community
      membership = CommunityMembership.new(:person => @person, :is_owner => params[:is_owner], :community => @current_community, :consent => @current_community.consent)

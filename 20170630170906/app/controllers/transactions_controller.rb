class TransactionsController < ApplicationController
  ...
  def new
    Result.all(
      ...
      transaction_params = HashUtils.symbolize_keys({listing_id: listing_model.id}.merge(params.slice(:start_on, :end_on, :quantity, :delivery, :cart)))
      submit_type = params[:submit_type] if PostPayTransactionsController::SUBMIT_TYPES.include?(params[:submit_type])
      ...
      when matches([:preauthorize, :stripe])
        #do nothing, because we never get here
        #Default sharetribe first creates a new form and then goes to post
        #In swapnights version the template is changed to post right away to create
        #In gostyling we also ignore new and go directly to create by posting the request form
      when matches([:postpay, __, true])
        if submit_type == PostPayTransactionsController::INQUIRY
          redirect_to post_pay_inquire_path(transaction_params)
        else
          redirect_to post_pay_book_path(transaction_params)
        end
  ...
  def create
    Result.all(
      ...
      ->(form, (listing_id, listing_model, author_model, process, gateway), _, _) {
        ...
        minimum_duration = TransactionService::Cart.get_or_create(listing: listing_model).minimum_duration
        quantity = Maybe(booking_fields).map { |b| DateUtils.duration_nights(b[:start_on], b[:end_on], minimum_duration) }.or_else(form[:quantity])
  ...
  def show
    ...
    messages_and_actions = TransactionViewUtils.merge_messages_and_transitions(
      ...
      TransactionViewUtils.payment_split_messages(transaction_conversation, conversation, tx_model.payment, @current_community.name_display_type))
    ...
    price_break_down_locals = nil
    payment_split_locals = nil

    if tx[:booking] && tx[:booking][:start_on]
      schedule = TransactionService::PaymentSchedule.get(community_id: tx[:community_id], transaction_id: tx[:id])
      payment_split = tx_model.payment.present? ? tx_model.payment.next_pending_split : nil
      price_break_down_locals = price_break_down_locals(tx, schedule)
      payment_split_locals = {
        braintree_form: Form::Braintree.new,
    ...
    render "transactions/show", locals: {
      ...
      payment_split_locals: payment_split_locals
    }
  end
  ...
  def price_break_down_locals(tx, schedule)
    if tx[:payment_process] == :none && tx[:listing_price].cents == 0
      ...
      due_now_sum = if tx[:payment_gateway] == :none
                      nil
                    else
      ...
      TransactionViewUtils.price_break_down_locals({
        transaction_status: tx[:current_state].to_s,
        form_action: booked_path(person_id: @current_user.id, listing_id: tx_model.listing[:id]),
        ...
        subtotal: tx[:item_total],
        shipping_price: tx[:shipping_price],
        total_label: total_label,
        total: tx[:payment_total],
        cart: cart,
  ...
  def render_free(listing_model:, author_model:, community:, params:)
    ...
    minimum_duration = TransactionService::Cart.get_or_create(listing: listing_model).minimum_duration
    duration = booking ? DateUtils.duration_nights(booking_start, booking_end, minimum_duration) : nil
    quantity = Maybe(booking ? DateUtils.duration_nights(booking_start, booking_end, minimum_duration) : TransactionViewUtils.parse_quantity(params[:quantity])).or_else(1)

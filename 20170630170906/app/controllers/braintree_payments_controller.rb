class BraintreePaymentsController < ApplicationController
  ...
  def edit
    ...
    payment_split = @braintree_payment.next_pending_split
    render locals: {
  ...
  def ensure_not_paid_already
    if @conversation.payment.status != "pending" && !(@conversation.payment.status == "partial" && @conversation.payment.next_pending_split.present?)

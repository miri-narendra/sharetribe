class ConfirmConversationsController < ApplicationController
  ...
  before_filter :ensure_is_starter, :only => [:confirm, :confirmation]
  ...
  def cancel
    if (@listing_transaction.starter != @current_user && @listing_transaction.author != @current_user)
       flash[:error] = "You cannot perform the requested action"
      return redirect_to (session[:return_to_content] || root)
    end
    ...
    payment_schedule = TransactionService::PaymentSchedule.get(community_id: @listing_transaction.community_id, transaction_id: @listing_transaction.id)
    
    if payment_schedule.paid_sum > 0
      return redirect_to cancel_refund_person_message_path(@current_user, id: @listing_transaction.id)
    end
    ...
    if @listing_transaction.author == @current_user
      status = :canceled
      complete_or_cancel_tx(@current_community.id, @listing_transaction.id, status, '', @current_user.id)
      flash[:notice] = t("layouts.notifications.offer_#{status}")
      return redirect_to person_transaction_path(:person_id => @current_user.id, :id => @listing_transaction.id)
    end

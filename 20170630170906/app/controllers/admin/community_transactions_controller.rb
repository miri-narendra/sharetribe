...
class Admin::CommunityTransactionsController < ApplicationController
  ...
  def index
    ...
    filter_params = transactions_filter_params

    conversations = if params[:sort].nil? || params[:sort] == "last_activity"
      TransactionQuery.transactions_for_community_sorted_by_activity_with_filter(
      ...
      TransactionQuery.transactions_for_community_sorted_by_column_with_filter(
        @current_community.id,
    ...
    count = TransactionQuery.transactions_count_for_community_with_filter(@current_community.id, filter_params)
  ...
  def transactions_filter_params
    ...
    params[:filter].delete(:starter) unless params[:filter][:starter].present?
    params[:filter].delete(:other_party) unless params[:filter][:other_party].present?
    p = params.require(:filter).permit(:starter, :other_party, :status, :payment_status) if params[:filter].present?
    p
  end
